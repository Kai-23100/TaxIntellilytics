...
# FLUTTERWAVE PAYMENT
# ================================
FLUTTERWAVE_SECRET_KEY = os.getenv("FLUTTERWAVE_SECRET_KEY", "FLWSECK_TEST-xxxxxxxxxxxx")
FLUTTERWAVE_PUBLIC_KEY = os.getenv("FLUTTERWAVE_PUBLIC_KEY", "FLWPUBK_TEST-xxxxxxxxxxxx")
...
def create_payment_link(username, amount=5000, currency="UGX"):
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            st.error("Payment initialization failed: %s" % response.text)
            return None
    except Exception as e:
        st.error(f"Payment creation error: {e}")
        return None
...
def qb_is_available_cached():
    try:
        import intuitlib, quickbooks  # noqa: F401
        return True
    except Exception:
        return False
...
DEFAULT_CONTROL_MAP = pd.DataFrame([
    ["Cash & Bank", "Debit", r"(?i)\bcash\b|bank|current account|cash at hand", 50000],
    ["Accounts Receivable", "Debit", r"(?i)\bar\b|trade receivable|debtors", 50000],
    ["Inventory", "Debit", r"(?i)\binventory\b|stock", 50000],
    ["Accounts Payable", "Credit", r"(?i)\bap\b|trade payable|creditors", 50000],
    ["VAT Payable", "Credit", r"(?i)\bvat\b|output vat|vat payable", 50000],
    ["VAT Receivable", "Debit", r"(?i)input vat|vat receivable", 50000],
    ["PAYE Payable", "Credit", r"(?i)paye\b|pay as you earn", 50000],
    ["WHT Receivable", "Debit", r"(?i)withholding tax (receivable|asset)|wht receivable|wht asset", 50000],
    ["Income Tax Payable", "Credit", r"(?i)income tax payable|corporation tax payable", 50000],
    ["Share Capital", "Credit", r"(?i)share capital|stated capital", 50000],
    ["Retained Earnings", "Credit", r"(?i)retained earnings|accumulated (profit|loss)", 50000],
...
def audit_findings(tb_df: pd.DataFrame,
    ...
    for _, r in ctrl.iterrows():
        if not bool(r["SignOK"]):
            exceptions.append({"Area": f"Control: {r['Category']}", "Issue": f"Sign mismatch (Expected {r['ExpectedSign']})", "Amount": float(r["MatchedAmount"])})

    for _, r in pl_comp.iterrows():
        if bool(r["Material"]):
            exceptions.append({"Area": f"P&L Reconciliation — {r['Item']}", "Issue": "Delta exceeds materiality", "Amount": float(r["Delta"])})
...
# ================================
# UI: Build full Streamlit app (tabs + sidebar)
# ================================
...
def show_tax_module():
    ...
    with tab3:
        ...
        st.dataframe(pd.DataFrame([{"Revenue": revenue, "COGS": cogs, "OPEX": opex, "Other Income": other_income, "Other Expenses": other_expenses, "PBIT": pbit}]).T, use_container_width=True)
